...
from clean_data import kde_peak
...
def import_distance_data():
    ...
    for file in glob.glob(os.path.join(folder, "*.csv")):
        ...
        df_filtered.insert(len(df_filtered.columns), 'DISTANCE', int(dis))
...
def clean(df):
    ...
    for name, group in grouped:
        d = kde_peak(group)
        d.insert(len(d.columns), 'DISTANCE', name)
        li.append(d)
    return pd.concat(li)
...
if __name__ == '__main__':
    ...
    plot_group_by(data_frame, grp_col='CHANNEL')
    export('grouped_by_freq_scatter_cleaned')
    plot_group_by(data_frame, grp_col='DISTANCE',
                  p1_x='CHANNEL', p1_x_label='Frequency (MHz)',
                  p2_x='CHANNEL', p2_x_label='Frequency (MHz)')
    export('grouped_by_dist_scatter_cleaned')
